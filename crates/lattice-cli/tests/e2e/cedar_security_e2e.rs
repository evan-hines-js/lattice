...
async fn test_cedar_security_e2e() {
    init_e2e_test();
    info!("Starting E2E test: Cedar Security Overrides");

    let result = tokio::time::timeout(E2E_TIMEOUT, run()).await;
    match result {
        Ok(Ok(())) => info!("TEST PASSED: cedar_security"),
        Ok(Err(e)) => {
            setup::cleanup_bootstrap_cluster(run_id()).await;
            panic!("Cedar Security E2E failed: {}", e);
        }
        Err(_) => {
            setup::cleanup_bootstrap_cluster(run_id()).await;
            panic!("Cedar Security E2E timed out after {:?}", E2E_TIMEOUT);
        }
    }
}
...
async fn run() -> Result<(), String> {
    ...
    integration::cedar_security::run_cedar_security_tests(result.ctx.require_workload()?).await?;
