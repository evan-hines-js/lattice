//! Fixed 10-service mesh bilateral agreement test
//!
//! Deploys a deterministic 10-service topology and verifies that bilateral
//! agreements (caller declares outbound + callee declares inbound) produce
//! the expected ALLOWED/BLOCKED traffic patterns.
//!
//! Service construction lives in `mesh_fixtures`, test utilities in `mesh_helpers`,
//! random mesh generation in `mesh_random`, and removal/mutation tests in `mesh_removal`.
...
///
/// Deploys all services, waits for pods, then retries verification every 15s
/// until it passes or 5 minutes elapse. Cleans up on success.
///
/// Edge removal/mutation testing lives in `mesh_removal.rs`.
