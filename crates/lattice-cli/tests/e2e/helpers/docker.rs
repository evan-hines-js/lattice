...
// =============================================================================
// Docker Container DNS Helpers
// =============================================================================

/// Resolve a Docker container's IP on the `kind` network.
///
/// Uses `docker inspect` to extract the IP address assigned to the container
/// on the `kind` bridge network.
pub async fn docker_container_ip(container_name: &str) -> Result<String, String> {
    let ip = run_cmd(
        "docker",
        &[
            "inspect",
...
/// Create a headless Service + EndpointSlice so CoreDNS resolves a Docker
/// container name to its real IP inside the cluster.
///
/// Docker containers on the `kind` network (e.g., `lattice-vault`, `lattice-keycloak`)
/// aren't in CoreDNS. This registers them so that:
/// - Pods resolve the name via normal DNS (returns real IP, not a VIP)
/// - Cilium `toFQDNs` caches the real IP and enforces correctly
/// - Istio ServiceEntry `resolution: DNS` resolves to the real IP
///
/// The Service is created in `namespace` so pods in that namespace can resolve
/// the name via their default search domain.
pub async fn ensure_docker_dns(
    kubeconfig: &str,
    container_name: &str,
    namespace: &str,
    port: u16,
) -> Result<(), String> {
    let ip = docker_container_ip(container_name).await?;
...
spec:
  ...
  ports:
    ...
    super::cedar::apply_yaml(kubeconfig, &svc_yaml).await?;

    let eps_yaml = format!(
        r#"apiVersion: discovery.k8s.io/v1
kind: EndpointSlice
metadata:
  name: {container_name}-docker
  namespace: {namespace}
  labels:
    kubernetes.io/service-name: {container_name}
    lattice.dev/docker-dns: "true"
addressType: IPv4
ports:
  - port: {port}
...
endpoints:
  - addresses:
    ...
    super::cedar::apply_yaml(kubeconfig, &eps_yaml).await?;

    info!(
        "Registered Docker DNS: {}.{}.svc → {} (port {})",
        container_name, namespace, ip, port
    );
    Ok(())
}

/// Delete the headless Service + EndpointSlice created by `ensure_docker_dns`.
pub async fn cleanup_docker_dns(kubeconfig: &str, container_name: &str, namespace: &str) {
    let _ = run_kubectl(&[
        "--kubeconfig",
        kubeconfig,
        "delete",
