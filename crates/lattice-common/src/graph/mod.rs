...
impl ServiceGraph {
    ...
    /// Insert or update a remote service discovered from LatticeClusterRoutes.
    ///
    /// Remote services are registered with `allows_all: true` because the remote
    /// cluster's `advertise: true` is implicit consent to inbound traffic.
    /// Local services declaring an `outbound` dependency to this name/namespace
    /// will get a ServiceEntry generated by the policy compiler.
    pub fn put_remote_service(
        ...
        let node = ServiceNode {
            ...
            dependencies: vec![],
            allowed_callers: HashSet::new(),
            allows_all: true, // advertise: true = implicit inbound consent
            depends_all: false,
            image: None,
            ports: BTreeMap::new(),
            selector: None,
