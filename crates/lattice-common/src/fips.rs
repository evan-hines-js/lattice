...
mod tests {
    ...
    #[test]
    fn add_root_install_env_adds_env_var() {
        let original = sample_deployment();
        let modified = add_root_install_env(&original);
        ...
        let has_root_install = env.iter().any(|e| {
            e.get("name").and_then(|n| n.as_str()) == Some("LATTICE_ROOT_INSTALL")
                && e.get("value").and_then(|v| v.as_str()) == Some("true")
        });
        assert!(
            has_root_install,
            "Should have LATTICE_ROOT_INSTALL=true env var"
        );
    }

    #[test]
    fn add_root_install_env_does_not_duplicate() {
        let original = sample_deployment();
        let modified = add_root_install_env(&original);
        let double_modified = add_root_install_env(&modified);
        ...
        let root_install_count = env
            .iter()
            .filter(|e| e.get("name").and_then(|n| n.as_str()) == Some("LATTICE_ROOT_INSTALL"))
            .count();
        assert_eq!(
            root_install_count, 1,
            "Should only have one LATTICE_ROOT_INSTALL env var"
        );
    }

    #[test]
    fn add_root_install_env_handles_invalid_json() {
        let invalid = "not valid json";
        let result = add_root_install_env(invalid);
        assert_eq!(result, invalid, "Should return original on parse error");
    }

    #[test]
    fn add_root_install_env_handles_non_deployment() {
        let namespace = serde_json::json!({
            "apiVersion": "v1",
            "kind": "Namespace",
        ...
        let result = add_root_install_env(&namespace);
        assert_eq!(result, namespace);
    }

    #[test]
    fn add_root_install_env_creates_env_array_if_missing() {
        ...
        let modified = add_root_install_env(&deployment_no_env);
        let parsed: serde_json::Value =
        ...
        assert_eq!(env.len(), 1);
        assert_eq!(
            env[0].get("name").and_then(|n| n.as_str()),
            Some("LATTICE_ROOT_INSTALL")
        );
    }
