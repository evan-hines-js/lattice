...
use lattice_operator::agent::connection::AgentRegistry;
use lattice_operator::agent::server::AgentServer;
use lattice_operator::bootstrap::{
    bootstrap_router, BootstrapState, CsrRequest, DefaultManifestGenerator,
};
use lattice_operator::pki::{AgentCertRequest, CertificateAuthority};
use lattice_operator::proto::agent_message::Payload;
use lattice_operator::proto::lattice_agent_client::LatticeAgentClient;
use lattice_operator::proto::{AgentMessage, AgentReady, AgentState};
...
async fn integration_bootstrap_http_full_flow() {
    ...
    let body = axum::body::to_bytes(response.into_body(), 1024 * 1024)
        .await
        .unwrap();
    let csr_response: lattice_operator::bootstrap::CsrResponse =
...
async fn integration_full_stack_bootstrap_to_ready() {
    ...
    let cert_der = lattice_operator::pki::parse_pem(&csr_response.certificate_pem).unwrap();
    let verification =
        lattice_operator::pki::verify_client_cert(&cert_der, ca.ca_cert_pem()).unwrap();
