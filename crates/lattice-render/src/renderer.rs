...
///
/// Go template syntax (`{{`, `}}`) in user content is escaped to prevent
/// injection into ESO's template engine. Only the `{{ .key }}` expressions
/// generated by this function for `${secret.*}` refs are left unescaped.
pub fn extract_secret_refs(content: &str, reverse_expand: bool) -> (String, Vec<FileSecretRef>) {
    ...
    // Phase 1: Escape any existing Go template delimiters in user content.
    // This prevents user-supplied `{{ .foo }}` from being evaluated by ESO.
    ...
    // Phase 2: Replace `${secret.RESOURCE.KEY}` with unescaped `{{ .eso_key }}`
