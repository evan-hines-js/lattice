...
///
/// Processes the batch inline (not spawned) to guarantee sequential execution.
/// The cell sends batches one at a time and waits for each ack, but processing
/// inline ensures the UID map is consistent even if the cell's behavior changes.
pub async fn handle(command_id: &str, batch: &MoveObjectBatch, ctx: &CommandContext) {
    let request_id = command_id.to_string();
    let target_namespace = &batch.target_namespace;
