...
pub async fn generate_capi_manifests(
    ...
    // Resolve credential secret name from InfraProvider for CAPI manifest generation
...
async fn get_or_create_bootstrap_token(
    ...
    // deadlock: basis only populates `BasisCluster.spec.serviceBlockCidr`
    // after the CR is applied, and the CR is applied with the bootstrap
    // token we're about to register. The token is consumed *after* CAPI
