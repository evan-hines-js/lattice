...
pub async fn handle_provisioning(cluster: &LatticeCluster, ctx: &Context) -> Result<Action, Error> {
    ...
    // Istiod needs the original direct API server kubeconfig. After
    // patch_kubeconfig_for_proxy_access rewrites the server URL, the
    // original is lost.
...
///
/// Istiod uses this kubeconfig for multi-cluster endpoint discovery. Using the
/// direct API server connection (instead of the auth proxy) avoids watch stream
/// issues during agent restarts.
